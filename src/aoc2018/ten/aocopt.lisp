...
;;the limiting factor, even with mutation,
;;is the lack of locality of reference
;;for our numeric types (I think).
;;Tested across multiple iteration
;;strategies, including mutable lists,
;;vectors, etc.  Can't break the 0.09s
;;barrier this way.
...
;; Thus Common Lisp does not have optimized storage layout for arrays
;; of structures, vectors, CLOS objects, etc. Since there is a pointer
;; for each element stored, access always needs an indirection and
;; there is nothing that will guarantee that these objects are stored
;; in linear order in memory.  Stored in linear order are the pointers
;; to them in the array.
...
;; ;;destructive map implementation.
;; (defun nmapcar (f xs)
;;   (declare (function f))
;;   (labels ((aux (ys)
;;              (let ((x (car ys)))
;;                (progn (setf (car ys) (funcall f x))
;;                       (if (cdr ys)
;;                           (aux (cdr ys))
;;                           xs)))))
;;     (aux xs)))

;; ;;mapcar is consing, which we could
;; ;;optimize further (likely yuge savings).
;; (defun evolve-state (state)
;;   (mapcar #'evolve-point state))

;; (defun evolve-state! (state)
;;   (nmapcar #'evolve-point state))

;; ;;use a vector instead of a list
;; (defun evolve-state-map (state)
;;   (map `(,'simple-vector ,(length state)) #'evolve-point state))
...
;;         do (setf (aref v i) (funcall f (aref v i)))
;;         finally (return v)))

;; (defun evolve-state-vec (state)
;;   (map `(,'simple-vector ,(length state)) #'evolve-point state))
