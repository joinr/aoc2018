...
;;Looks like a simple vector sum...

(def deltas
  (->> (slurp "day-1-input.txt")
       clojure.string/split-lines
       (map clojure.edn/read-string)))


(def res (reduce + 0 deltas))
...
;;brute-force solution is to walk straight through....
(defn dupe [init xs]
  (reduce (fn [[n visited :as acc] delta]            
            (let [k (+ delta n)]
              (if (visited k) (reduced [:found k visited])
                  [k (conj visited k)])))
          [init #{init}] (cycle xs)))

#_(defn freqs [init xs]
  (reductions (fn [n delta]
                (+ n delta)) init (cycle xs)))

#_(let [visited (atom #{})
      seen? (fn [n] (if (@visited n)
                      n
                      (do (swap! visited conj n)
                          nil)))]
  (some seen?  (freqs 0 deltas)))
