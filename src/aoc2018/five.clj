...
(def sample-data "dabAcCaCBAcCcaDA")

;;DON'T FORGET TO TRIM YOUR INPUT, \r\n at the end
;;cost me!
(defn read-sequence
  ([]  (clojure.string/trim (slurp "day-5-input.txt"))))

(def units [\a \b \c \d \e \f \g \h \i
            \j \k \l \m \n \o \p \q \r
            \s \t \u \v \w \x \y \z])

(def opposites
  (let [m  (->> (for [x units]
                  [x (first (clojure.string/upper-case x))])
                (into {}))]    
    (reduce-kv (fn [acc from to]
                 (assoc acc to from)) m m)))

;;could memoize this...
(defn cancels? [l r]
  (identical? (opposites l) r))

;;find the next reaction
;;if we're smart, we can optimize
;;the break detection by factoring out
;;like stuff...
;;if we have pairs of cancellations,
;;we can just keep drawing them into the break.
;;we can effectively elide them while scanning.

;;(last ....) is O(n) due to seq traversal.
;;we're using vectors intentionally for fast tail
;;access.
(defn last-vec [^clojure.lang.PersistentVector v]
  (let [c (.count v)]
    (when (pos? c) ;;keep semantics of (last (seq v))
      (.nth v (unchecked-dec c)))))

(defn react [xs]
  (->> (iterate (fn [[scanned remaining]]
                  (when (seq remaining)
                    (if (cancels? (last-vec scanned) (first remaining))
                      [(pop scanned)
                       (rest remaining)]
                      [(conj scanned (first remaining))
                       (rest remaining)])))
                [[] (seq xs)])       
       (take-while identity)
       (map first)
...
;;in this case - the data has fairly even distribution of all
;;letters, and each polarity is presence in equal numbers.

;;just BF* the thing.
(defn filtered-polymer [unit xs]
  (let [include (complement #{unit (opposites unit)})]
    [unit (react (filter include xs))]))

(defn all-polys [xs]
  (->>   (for [u units]
           (filtered-polymer u xs))
         (sort-by second)))
